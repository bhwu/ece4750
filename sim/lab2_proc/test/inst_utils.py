...
# gen_jal_template
#-------------------------------------------------------------------------
# Template for jal instructions. We test two forward
...
# that test also makes sure that the instruction in the jump delay slot
# is _not_ executed.
...
def gen_jal_template(
  ...
  inst, reg_dest, result_pc_start
):

  # Determine the expected control flow pattern
  control_flow_pattern = 0b101010
  # Determine the result PC
    
  result_pc = result_pc_start + 4 * (num_nops_src0 + num_nops_src1) + 0x0014
...
{id_c}:                                   
    ...
    csrw   proc2mngr, {reg_dest} > {result_pc}
...
def gen_jal_value_test( inst, num_nops_src0, num_nops_src1, reg_dest ,result_pc_start ):
  return gen_jal_template(  num_nops_src0, num_nops_src1,
  inst, reg_dest , result_pc_start)
  
#-------------------------------------------------------------------------
# gen_jalr_template
#-------------------------------------------------------------------------
# Template for jalr instructions. We test two forward
# jumps and one backwards jump. The way we actually do the test is
# we update a register to reflect the control flow; certain bits in this
# register are set at different points in the program. Then we can check
# the control flow bits at the end to see if only the bits we expect are
...
def gen_jalr_template(
  num_nops_src0, num_nops_src1,
  inst, reg_src, reg_dest, result_pc_start
):
  ...
  result_pc = result_pc_start + 4 * (num_nops_src0 + num_nops_src1) + 0x0024

  # Create unique labels

  global gen_jalr_template_id
  id_a = "label_{}".format( gen_jalr_template_id + 1 )
  id_b = "label_{}".format( gen_jalr_template_id + 2 )
  id_c = "label_{}".format( gen_jalr_template_id + 3 )
  ...
  return """
    ...
    addi   x3, x0, 0                                     # 0x0200
                                                         #
    lui  {reg_src},            %hi[{id_a}]               # 0x0204
    addi {reg_src}, {reg_src}, %lo[{id_a}]               # 0x0208

    {nops_src0}                            

    {inst} {reg_dest}, {reg_src}, 0 
    addi   x3, x3, 0b000001               
...
def gen_jalr_value_test( inst, num_nops_src0, num_nops_src1, reg_src, reg_dest ,result_pc_start ):
  return gen_jalr_template(  num_nops_src0, num_nops_src1,
  inst, reg_src, reg_dest , result_pc_start)
...
# gen_sw_template
#-------------------------------------------------------------------------
# Template for store instructions. We first write the two base registers before
# executing the instruction under test. We parameterize the number of
# nops after writing the base register and the instruction under test to
# enable using this template for testing various bypass paths. We also
# parameterize the register specifiers to enable using this template to
...
def gen_sw_template(
  ...
  return """
    ...
    # Move base value into register
    csrr {reg_base0}, mngr2proc < {base0}
    csrr {reg_base1}, mngr2proc < {base1}
    {nops_base}

    # Instruction under test
    {inst} {reg_base1}, {offset}({reg_base0})
    {nops_dest}
    
    # Check the result
    lw x3, {offset}({reg_base0})
    csrw proc2mngr, x3 > {result}
...
def gen_sw_dest_dep_test( num_nops, inst, base0, base1, result ):
  return gen_sw_template( 8, num_nops, "x1", "x2", inst, 0, base0, base1, result )
...
def gen_sw_base_dep_test( num_nops, inst, base0, base1, result ):
  return gen_sw_template( num_nops, 0, "x1", "x2", inst, 0, base0, base1, result )
...
def gen_sw_base_eq_dest_test( inst, base, result ):
  return gen_sw_template( 0, 0, "x3", "x3", inst, 0, base, base, result )
...
def gen_sw_value_test( inst, offset, base0, base1, result ):
  return gen_sw_template( 0, 0, "x1", "x2", inst, offset, base0, base1, result )
